...
static volatile float s_line_bias_rpm;
                         ...
                         uint8_t integrate)
{
    ...
    /* Do not keep integrating in the direction that is already saturated. */
    if (integrate != 0U) {
...
{
    return s_left_target_rpm;
}

float Control_GetRightTargetRpm(void)
{
    return s_right_target_rpm;
}
...
{
    return LineSensor_Read();
}

uint32_t Control_GetTickCount(void)
{
    return s_control_ticks;
}
...
void Control_Tick(void)
{
    ...
    /* Physical mapping: J2/PWM B is the left wheel and uses encoder B;
     * J1/PWM A is the right wheel and uses encoder A. */
    ...
    if (++s_speed_sample_ticks >= SPEED_SAMPLE_TICKS) {
        /* The vehicle is forward-only, so speed feedback uses magnitude. */
        s_left_rpm = Encoder_CountsToRpm(s_left_speed_counts,
                                         SPEED_SAMPLE_MS);
        s_right_rpm = Encoder_CountsToRpm(s_right_speed_counts,
                                          SPEED_SAMPLE_MS);
    ...
    if (s_mode == CTRL_STRAIGHT) {
        ...
        if (LineSensor_GetSteeringError(&line_error) == 0U) {
            /* Empty, fully covered, and non-contiguous states drive straight. */
            s_line_error = 0;
            ...
            target_left = s_base_rpm + s_line_bias_rpm + correction;
            target_right = s_base_rpm - s_line_bias_rpm - correction;
        }
    }
    ...
    /* Recalculate PWM every 10 ms for prompt steering. Integrate only when a
     * new 50 ms encoder measurement is available. */
