...
- `SW2/RESET` is connected to the MCU `NRST` pin and remains a hardware reset.
  It is not sampled as a GPIO and does not require application code.
- `SW3/BLS` is connected to `PA18`, uses the board pull-down, and reads high
  while pressed. The application debounces its release as one click.
- A single BLS click starts the selected competition mode. Two BLS clicks
  within the double-click window select the next mode instead.
- `PB8` is the S28A expansion key input and is left available; it is not the
  C07A RESET signal.
